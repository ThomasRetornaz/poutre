...
namespace poutre
{
  ...
  public:
    ...
    DenseImage (const std::vector<size_t>& dims) : m_data (nullptr), m_coordinnates (), m_allocator (), m_numelemwithpaddingifany (0)
    {
    ...
    DenseImage (const std::initializer_list<size_t>& dims) : m_data (nullptr), m_coordinnates (), m_allocator (), m_numelemwithpaddingifany (0)
    {
      if (dims.size () != m_numdims) POUTRE_RUNTIME_ERROR ("Invalid input initializer regarding NumDims of DenseImage container");
      //std::copy(dims.begin( ), dims.end( ), m_coordinnates.begin( ));
      auto it = dims.begin ();
    ...
    pointer
    datas () POUTRE_NOEXCEPT
    {
      return m_data;
    }

    const_pointer
    datas () const POUTRE_NOEXCEPT
    {
      return m_data;
    }
    ...
    void
    swap (self_type& rhs) //POUTRE_NOEXCEPT(POUTRE_NOEXCEPT (swap(declval<value_type&>( ), declval<value_type&>( )))) //wait MSVC2013 noexcept impl ...
    {
      if (this != &rhs)
        {
          using std::swap;
    ...
    CompoundType
    GetCType () const POUTRE_NOEXCEPT override
    {
      return m_ctype;
    }

    PType
    GetPType () const POUTRE_NOEXCEPT override
    {
      return m_ptype;
    }

    ImgType
    GetImgType () const POUTRE_NOEXCEPT override
    {
      return m_imgtype;
    }
  ...
  protected:
    ...
    DenseImage (const self_type& rhs)
            : m_data (nullptr), m_numelemwithpaddingifany (rhs.m_numelemwithpaddingifany), m_coordinnates (rhs.m_coordinnates), m_allocator (rhs.m_allocator)
    {
      m_data = m_allocator.allocate (m_numelemwithpaddingifany);
  ...
  public:
    ...
    DenseImage (self_type&& rhs) : m_data (nullptr), m_numelemwithpaddingifany (0), m_coordinnates (), m_allocator ()
    {
      *this = std::move (rhs);
    }
