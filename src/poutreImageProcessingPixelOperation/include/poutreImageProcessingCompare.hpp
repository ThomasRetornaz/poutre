...
namespace poutre
{
   ...
   struct OpCompEqualValue
   {
      ...
      {
         return  simd::is_equal(lhs, m_val_pack);
      }
   };
#else
	template< typename T1>
   ...
   struct OpCompDiffValue
   {
      ...
      POUTRE_ALWAYS_INLINE simd_mask_t operator()(const U& lhs) const POUTRE_NOEXCEPT
      {
         return  simd::is_not_equal(lhs, m_val_pack);
      }
   };
#else
	//!Operator diff
   ...
   struct OpCompSupValue
   {
      ...
      POUTRE_ALWAYS_INLINE simd_mask_t operator()(const U& lhs) const POUTRE_NOEXCEPT
      {
         return  simd::is_greater(lhs, m_val_pack);
      }
   };
#else
	//!Operator Sup
   ...
   struct OpCompSupEqualValue
   {
      ...
      POUTRE_ALWAYS_INLINE simd_mask_t operator()(const U& lhs) const POUTRE_NOEXCEPT
      {
         return  simd::is_greater_equal(lhs, m_val_pack);
      }
   };
#else
	//!Operator SupEqual
   ...
   struct OpCompInfValue
   {
      ...
      POUTRE_ALWAYS_INLINE simd_mask_t operator()(const U& lhs) const POUTRE_NOEXCEPT
      {
         return  simd::is_less(lhs, m_val_pack);
      }
   };
#else
	//!Operator Inf
   ...
   struct OpCompInfEqualValue
   {
      ...
      POUTRE_ALWAYS_INLINE simd_mask_t operator()(const U& lhs) const POUTRE_NOEXCEPT
      {
         return  simd::is_less_equal(lhs, m_val_pack);
      }
   };
   ...
   struct compare_sss_simd
   {
      ...
      compare_sss_simd(const Tout& i_valtrue, const Tout& i_valfalse) : cop(), m_valtrue_pack(i_valtrue), m_valfalse_pack(i_valfalse), m_valtrue(i_valtrue), m_valfalse(i_valfalse) {}
      compare_sss_simd(const Tout& i_valtrue, const Tout& i_valfalse, const CompareOp& op) :cop(op), m_valtrue_pack(i_valtrue), m_valfalse_pack(i_valfalse), m_valtrue(i_valtrue), m_valfalse(i_valfalse) {}
      ...
      {
         return simd::if_else(cop(i_val), m_valtrue_pack, m_valfalse_pack);
      }
	...
	struct viewCompare_sss
	{
		...
		{
			/*std::cout << "***********************************************************************************" << std::endl;
			std::cout << "call viewCompare_sss array view template specialization, fall back ptrTime" << std::endl;
			std::cout << "***********************************************************************************" << std::endl;*/
	...
	template<typename Tin, typename Tcomp, typename Ttrue, typename Tfalse, typename Tout, ptrdiff_t Rank, template <typename, ptrdiff_t> class ViewIn, template <typename, ptrdiff_t> class ViewComp, template <typename, ptrdiff_t> class ViewTrue, template <typename, ptrdiff_t> class ViewFalse, template <typename, ptrdiff_t> class ViewOut>
	void ViewCompare_iii_func_helper(
		const ViewIn<Tin, Rank>& i_vin,
		CompOpType compOpType,
		const ViewComp<Tcomp, Rank>& i_vcomp,
		const ViewTrue<Ttrue, Rank>& i_vtrue,
	...
	void ViewCompare_sis_func_helper(
		...
		ViewOut<Tout, Rank>& o_vout)
	{
		return ViewCompare_sis_dispatch(
			i_vin,
			compOpType,
			i_compval,
			i_vtrue,
	...
	void ViewCompare_isi_func_helper(
		...
		ViewOut<Tout, Rank>& o_vout)
	{
		return ViewCompare_isi_dispatch(
			i_vin,
			compOpType,
			i_vcomp,
			i_valtrue,
	...
	void ViewCompare_iis_func_helper(
		...
		ViewOut<Tout, Rank>& o_vout)
	{
		return ViewCompare_iis_dispatch(
			i_vin,
			compOpType,
			i_vcomp,
			i_vtrue,
	...
	void ViewCompare_ssi_func_helper(
		...
		ViewOut<Tout, Rank>& o_vout)
	{
		return ViewCompare_ssi_dispatch(
			i_vin,
			compOpType,
			i_compval,
			i_valtrue,
	...
	void ViewCompare_iss_func_helper(
		...
		ViewOut<Tout, Rank>& o_vout)
	{
		return ViewCompare_iss_dispatch(
			i_vin1,
			compOpType,
			i_vin2,
			i_valtrue,
