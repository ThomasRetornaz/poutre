...
namespace poutre
{
  ...
  struct PixelWiseBinaryOpDispatcher<T1, T2, Tout, Rank, array_view, array_view, array_view, BinOp>
  {
    ...
    {
      // std::cout << "\n" << "call PixelWiseBinaryOpDispatcher array view template specialization
      // array view ptr";
      POUTRE_ASSERTCHECK(i_vin1.size() == i_vin2.size(), "Incompatible views size");
      POUTRE_ASSERTCHECK(o_vout.size() == i_vin2.size(), "Incompatible views size");
  ...
  template<typename T1,
           ...
           template<typename, ptrdiff_t>
           class ViewOut,
           template<typename, typename, typename, class TAG>
           class BinOp,
           typename = void>
  ...
  {
    void
    operator()(const array_view<T1, Rank> &i_vin1, const array_view<T2, Rank> &i_vin2, array_view<Tout, Rank> &o_vout) const
    {
      POUTRE_ASSERTCHECK(i_vin1.size() == i_vin2.size(), "Incompatible views size");
      POUTRE_ASSERTCHECK(o_vout.size() == i_vin2.size(), "Incompatible views size");
      // get the specialized operator
      using real_op = /*typename*/ BinOp<T1, T2, Tout, tag_SIMD_disabled>;
      real_op op;
      // std::cout << "\n" << "call PixelWiseBinaryOpDispatcherWithTag array view template
      // specialization same type,fall back ptr";
      auto i_vinbeg1 = i_vin1.data();
      auto i_vinend1 = i_vin1.data() + i_vin1.size();
      auto i_vinbeg2 = i_vin2.data();
      auto i_voutbeg = o_vout.data();
