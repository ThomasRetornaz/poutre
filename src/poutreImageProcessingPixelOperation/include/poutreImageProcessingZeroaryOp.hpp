...
namespace poutre
{
  ...
  struct ZeroaryOpDispatcher<T1, T2, Rank, array_view, array_view, UnOp>
  {
    ...
    {
      // std::cout << "\n" << "call UnaryOpDispatcher array view template specialization,fall back
      // ptr";
  ...
  template<typename T, ptrdiff_t Rank, class UnOp> struct ZeroaryOpDispatcher<T, T, Rank, array_view, array_view, UnOp>
  {
    void operator()(const array_view<T, Rank> &i_vin, array_view<T, Rank> &o_vout, UnOp &op) const
    {
      // std::cout << "\n" << "call UnaryOpDispatcher array view template specialization same
