...
namespace poutre
{
    ...
    template <typename T1, typename T2, typename T3, typename Tout, ptrdiff_t Rank,
              ...
              template <typename, ptrdiff_t> class View3, template <typename, ptrdiff_t> class ViewOut, class TerOp>
    struct PixelWiseTernaryOpDispatcher
    {
        ...
        void operator()(const View1<T1, Rank> &i_vin1, const TerOp &op, const View2<T2, Rank> &i_vin2,
                        const View3<T3, Rank> &i_vin3, ViewOut<Tout, Rank> &o_vout) const
        {
            ...
            if (vInbound1 == vOutbound && vInbound2 == vOutbound && vInbound1 == vInbound3 &&
                stridevIN1 == stridevOut && stridevIN2 == stridevOut &&
                stridevIN1 == stridevIN3) // same bound + same stride -> one idx
            {
    ...
    struct PixelWiseTernaryOpDispatcher<T1, T2, T3, Tout, Rank, array_view, array_view, array_view, array_view, TerOp>
    {
        ...
        void operator()(const array_view<T1, Rank> &i_vin1, const TerOp &op, const array_view<T2, Rank> &i_vin2,
                        const array_view<T3, Rank> &i_vin3, array_view<Tout, Rank> &o_vout) const
        {
    ...
    template <typename T1, typename T2, typename T3, typename Tout, ptrdiff_t Rank,
              template <typename, ptrdiff_t> class View1, template <typename, ptrdiff_t> class View2,
              template <typename, ptrdiff_t> class View3, template <typename, ptrdiff_t> class ViewOut, class TerOp>
    void PixelWiseTernaryOp(const View1<T1, Rank> &i_vin1, const TerOp &op, const View2<T2, Rank> &i_vin2,
                            const View3<T3, Rank> &i_vin3, ViewOut<Tout, Rank> &o_vout) POUTRE_NOEXCEPTONLYNDEBUG
    {
