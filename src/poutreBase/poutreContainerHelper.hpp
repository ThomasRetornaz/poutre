...
#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable : 4244) // 4244 'conversion' conversion from 'type1' to 'type2', possible loss of data -> due to
                                // cast in operator / %  ...
#endif
...
namespace poutre
{
    ...
    namespace details
    {
        ...
        // GCC5.1 seems ok ,but fail for Visual 2015 .... see
        // http://blogs.msdn.com/b/vcblog/archive/2015/04/29/c-11-14-17-features-in-vs-2015-rc.aspx Those specialization
        // and design was inspired by ms amp tricks
        ...
        {
            POUTRE_ALWAYS_INLINE static POUTRE_CXX14_CONSTEXPR void op(const value_type &A0, const value_type &A1,
                                                                       value_type &res) POUTRE_NOEXCEPT
            {
                res = (A0 + A1);
            }
        };
        ...
        {
            POUTRE_ALWAYS_INLINE static POUTRE_CXX14_CONSTEXPR bool op(const value_type &A0,
                                                                       const value_type &A1) POUTRE_NOEXCEPT
            {
                return (A0 == A1);
            }
        };
        ...
        template <class container, ptrdiff_t Rank = container::rank> struct helper_comp_lexicographic_less_container_op
        {
            ...
            POUTRE_ALWAYS_INLINE static POUTRE_CXX14_CONSTEXPR bool op(container const &A0,
                                                                       container const &A1) POUTRE_NOEXCEPT
            {
        ...
        {
            using value_type = typename container::value_type;
            POUTRE_ALWAYS_INLINE POUTRE_CXX14_CONSTEXPR void op(container const &A0, container const &A1,
                                                                container &res) POUTRE_NOEXCEPT
            {
                for (size_t i = 0; i < Rank; i++)
                {
        ...
        {
            using value_type = typename container::value_type;
            POUTRE_ALWAYS_INLINE static POUTRE_CXX14_CONSTEXPR void op(container const &A0, container const &A1,
                                                                       container &res) POUTRE_NOEXCEPT
            {
        ...
        struct helper_assign_container_op
        {
            ...
            POUTRE_ALWAYS_INLINE static POUTRE_CXX14_CONSTEXPR void op(containerother const &A0,
                                                                       container &res) POUTRE_NOEXCEPT
            {
                for (size_t i = 0; i < Rank; i++)
                {
        ...
        struct helper_assign_container_valueop
        {
            ...
            POUTRE_ALWAYS_INLINE static POUTRE_CXX14_CONSTEXPR void op(othervaluetype const &A0,
                                                                       container &res) POUTRE_NOEXCEPT
            {
                for (size_t i = 0; i < Rank; i++)
                {
        ...
        template <class bnds, class idx, ptrdiff_t Rank = bnds::rank> struct helper_contains_container_op
        {
            POUTRE_ALWAYS_INLINE static POUTRE_CXX14_CONSTEXPR bool op(bnds const &bnd, idx const &id) POUTRE_NOEXCEPT
            {
                static_assert(bnds::rank == idx::rank,
                              "helper_contains_container_op: all provided container must share same rank");
                for (size_t i = 0; i < Rank; i++)
                {
                    if (id[i] < 0)
                        return false;
        ...
        template <class bnds, class idx> struct helper_contains_container_op<bnds, idx, 3>
        {
            POUTRE_ALWAYS_INLINE static POUTRE_CXX14_CONSTEXPR bool op(bnds const &bnd, idx const &id) POUTRE_NOEXCEPT
            {
                return (id[0] >= 0) && (id[0] < bnd[0]) && (id[1] >= 0) && (id[1] < bnd[1]) && (id[2] >= 0) &&
                       (id[2] < bnd[2]);
            }
        };
        ...
        {
            POUTRE_ALWAYS_INLINE static POUTRE_CXX14_CONSTEXPR offset op(bnds const &i_bnd,
                                                                         idx const &i_idx) POUTRE_NOEXCEPT
            {
        ...
        {
            POUTRE_ALWAYS_INLINE static POUTRE_CXX14_CONSTEXPR offset op(const bnds &i_bnd,
                                                                         const idx &i_idx) POUTRE_NOEXCEPT
            {
        ...
        {
            POUTRE_ALWAYS_INLINE static POUTRE_CXX14_CONSTEXPR void op(bnds const &i_bnd, offset off,
                                                                       idx &o_idx) POUTRE_NOEXCEPTONLYNDEBUG
            {
                static_assert(bnds::rank == idx::rank, "get_offset: all provided container must share same rank");
        ...
        {
            POUTRE_ALWAYS_INLINE static POUTRE_CXX14_CONSTEXPR void op(bnds const &i_bnd, idx const &i_idx,
                                                                       offset shift, idx &o_idx) POUTRE_NOEXCEPT
            {
