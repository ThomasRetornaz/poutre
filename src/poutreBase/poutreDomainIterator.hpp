...
public:
  using parent = std::iterator<std::bidirectional_iterator_tag, DataType,
                               std::ptrdiff_t, DataType *, DataType &>;
...
public:
  pdomain_iterator(DataType *ptr, bounds<Rank> bnd, index<Rank> strides,
                   ptrdiff_t currentiter)
      : m_ptr(ptr), m_boundit(bnd), m_strides(strides),
        m_currentiter(currentiter), m_enditer(bnd.size()) {}
  ...
  bool operator==(const self_type &rhs) const {
    return (m_ptr == rhs.getPtr() && m_currentiter == rhs.m_currentiter &&
            m_enditer == rhs.m_enditer);
  }
  bool operator!=(const self_type &rhs) const {
    return (m_ptr != rhs.getPtr() || m_currentiter != rhs.m_currentiter ||
            m_enditer != rhs.m_enditer);
  ...
  reference operator*() { return m_ptr[getOffset()]; }
  const reference operator*() const { return m_ptr[getOffset()]; }
  pointer operator->() { return &m_ptr[getOffset()]; }
