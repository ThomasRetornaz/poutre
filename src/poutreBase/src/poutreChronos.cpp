...
namespace poutre
{
  ...
  const Timer::timerep Timer::GetCumulativeTime() const POUTRE_NOEXCEPT { return m_accu; }
  ...
  const std::size_t Timer::NbIter() const POUTRE_NOEXCEPT { return m_nbiter; }
  ...
  //! ctor
  ScopedTimer::ScopedTimer(Timer &itimer) : m_timer(itimer) { m_timer.Start(); }
  //! dtor
  ScopedTimer::~ScopedTimer(void) { m_timer.Stop(); }
