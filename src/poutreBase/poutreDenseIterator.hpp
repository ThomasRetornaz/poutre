...
class pdense_iterator
    : public std::iterator<contiguous_iterator_tag, DataType, std::ptrdiff_t,
                           DataType *, DataType &> {
public:
  using parent = std::iterator<contiguous_iterator_tag, DataType,
  ...
  pdense_iterator(DataType *ptr = nullptr, DataType *ptrorig = nullptr)
...
public:
  ...
  pdense_reverse_iterator(DataType *ptr = nullptr, DataType *ptrorig = nullptr)
      : parent(ptr, ptrorig) {}
  ...
  ptrdiff_t
  operator-(const self_type &rawReverseIterator) { /*todo assert same orig?*/
