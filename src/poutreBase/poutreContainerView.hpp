 ...
 * @brief Implement View and StridedView as defined in
...
POUTRE_CXX14_CONSTEXPR ptrdiff_t view_offset(const index<Rank> &idx,
                                             const index<Rank> &stride) {
  ptrdiff_t offset{};
...
POUTRE_CXX14_CONSTEXPR T &view_access(T *data, const index<Rank> &idx,
                                      const index<Rank> &stride) {
...
template <typename Viewable, typename U,
          typename View = std::remove_reference_t<Viewable>>
using is_viewable_on_u = std::integral_constant<
    bool, std::is_convertible<typename View::size_type, ptrdiff_t>::value &&
              std::is_convertible<typename View::value_type *,
                                  std::add_pointer_t<U>>::value &&
 ...
 * Implement View and StridedView as defined in
...
/**
 * @brief An array_view is a potentially multidimensional view on a sequence of
 * uniformly strided objects of a uniform type, contiguous in the least
...
public:
  ...
  POUTRE_CXX14_CONSTEXPR array_view()
      POUTRE_NOEXCEPT_IF(POUTRE_NOEXCEPT_EXPR(bounds_type()) &&
                         POUTRE_NOEXCEPT_EXPR(index_type()))
  ...
  POUTRE_CXX14_CONSTEXPR array_view(Viewable &&vw)
      : m_bnd(vw.size()), m_data(vw.data()) {
    static_assert(Rank == 1,
                  "array_view(Viewable&& vw) is only allowed for rank=1 view");
  }
  ...
  //! ctor from other array_view with AnyRank is only allowed for flattened view
  //! (ie Rank=1)
  //@warning only available for AnyRank==1
  POUTRE_CONSTEXPR array_view(const array_view<U, R> &rhs) POUTRE_NOEXCEPT
      : m_bnd(rhs.m_bnd.size()),
        m_data(rhs.m_data) {}

  //! ctor from C_Array
  //@warning only available for rank==1
  template <
      size_t Extent, ptrdiff_t rank = Rank,
  ...
  POUTRE_CONSTEXPR array_view(const array_view<U, Rank> &rhs)
      POUTRE_NOEXCEPT_IF(POUTRE_NOEXCEPT_EXPR((m_bnd(rhs.m_bnd))) &&
                         POUTRE_NOEXCEPT_EXPR((m_stride_idx(rhs.m_stride_idx))))
  ...
  POUTRE_CXX14_CONSTEXPR array_view(Viewable &&vw, bounds_type bound)
      : m_bnd(bound), m_data(vw.data()) {
    if (vw.size() < bound.size()) {
      POUTRE_RUNTIME_ERROR("array_view(Viewable&& vw, bounds_type bound) can't "
                           "create view, provided bounds overfit vw");
    }
  }
  ...
  array_view(pointer ptr, bounds_type bound) : m_bnd(bound), m_data(ptr) {
    if (!m_data && bound.size() != 0) {
      POUTRE_RUNTIME_ERROR("array_view(pointer ptr, bounds_type bound) null "
                           "ptr is allowed only if bound.size()==0");
    }
    // details::helper_assign_container_valueop<index_type,
    // AssignOpType::AssignOp, Rank>::op (1, m_stride_idx);
  }
  ...
  template <
      ...
      typename std::enable_if<
          ...
          std::is_same<typename std::remove_cv<U>::type,
                       typename std::remove_cv<value_type>::type>::value>::type
          * = nullptr>
  //! Assignement operator
  POUTRE_CXX14_CONSTEXPR self_type &
  operator=(const array_view<U, Rank> &rhs) POUTRE_NOEXCEPT {
  ...
  POUTRE_CONSTEXPR bounds_type bound() const POUTRE_NOEXCEPT { return m_bnd; }
  ...
  //! Getter raw access to underlying data ptr to the contiguous sequence on
  //! which the view was created.

  POUTRE_CONSTEXPR pointer data() const POUTRE_NOEXCEPT { return m_data; }
  /**@}*/
  ...
  POUTRE_CXX14_CONSTEXPR reference
  operator[](const index_type &idx) const POUTRE_NOEXCEPTONLYNDEBUG {
   ...
   */
  /**@{*/
  //! Return a view such that the initial element is (*this)[{slice, 0, 0, �...,
  ...
  operator[](difference_type slice) const POUTRE_NOEXCEPTONLYNDEBUG {
    ...
    // Compute prt shift
    index_type idxslice;
    details::helper_assign_container_valueop<index_type, AssignOpType::AssignOp,
                                             Rank>::op(0, idxslice);
    idxslice[0] = slice;
    pointer data_slice =
        m_data +
        details::get_offset_from_coord_nostride<bounds_type, index_type>::op(
            m_bnd, idxslice);
  ...
  section(const index_type &origin, const bounds_type &section_bnd) const {
    // precondition
    POUTRE_ASSERTCHECK(m_bnd.contains(origin),
                       "section(origin,section_bnd) origin is Out of bound");
    index_type shifted = section_bnd + origin;
    shifted -= 1;
    POUTRE_ASSERTCHECK(m_bnd.contains(shifted),
                       "section(origin,section_bnd) section_bnd shifted by "
                       "origins is out of bound");
  ...
  section(const index_type &origin) const {
    ...
    // std::cout<<"\nstrided_array_view::section init "<<m_bnd<<" sub
    // "<<section_bnd<<" stride"<<stride()<<"\n";
    pointer data_section =
        m_data +
        details::get_offset_from_coord_nostride<bounds_type, index_type>::op(
            m_bnd, origin);
...
public:
  POUTRE_CXX14_CONSTEXPR T &
  operator[](const idx2d &idx) const POUTRE_NOEXCEPTONLYNDEBUG {
    POUTRE_ASSERTCHECK(m_bnd.contains(idx) == true, "Out of bound");
...
public:
  ...
  POUTRE_CONSTEXPR strided_array_view(const array_view<U, Rank> &rhs)
      POUTRE_NOEXCEPT_IF(POUTRE_NOEXCEPT_EXPR(m_bnd(rhs.m_bnd)) &&
                         POUTRE_NOEXCEPT_EXPR(m_stride_idx()))
  ...
  POUTRE_CONSTEXPR strided_array_view(const strided_array_view<U, Rank> &rhs)
      POUTRE_NOEXCEPT_IF(POUTRE_NOEXCEPT_EXPR(m_sview_bnd(rhs.m_bnd)) &&
                         POUTRE_NOEXCEPT_EXPR(m_stride_idx(rhs.m_stride_idx)))
      : m_bnd(rhs.m_bnd), m_stride_idx(rhs.stride()), m_data(rhs.m_data) {}
  ...
  strided_array_view(pointer ptr, bounds_type bounds, index_type stride)
    ...
    if (!m_data && bounds.size() != 0) {
      POUTRE_RUNTIME_ERROR("array_view(pointer ptr, bounds_type bounds) null "
                           "ptr is allowed only if bound.size()==0");
    }
    //      if (!bounds.contains (stride))
    //        {
    //          POUTRE_RUNTIME_ERROR ("strided_array_view(pointer ptr,
    //          bounds_type bounds, index_type stride) provided stride is out of
    //          bounds");
    //        }
  }
  ...
  //! Assignement operator
  POUTRE_CXX14_CONSTEXPR self_type &
  operator=(const strided_array_view<U, Rank> &rhs) POUTRE_NOEXCEPT {
    m_bnd = rhs.m_bnd;
   ...
   */
  /**@{*/
  //! Return A strided view such that the initial element is(*this)[{slice, 0,
  //! 0, �..., 0}], the bounds are{bounds( )[1], bounds( )[2],
  //! �...,bounds()[Rank - 1] }, and the stride is{ stride( )[1], stride( )[2],
  //! �...,stride()[Rank - 1] }
  ...
  section(const index_type &origin, const bounds_type &section_bnd) const {
    ...
    index_type shifted = section_bnd + origin;
    shifted -= 1;
    // std::cout<<"\n strided_array_view::section with bound
    // shifted:"<<shifted<<" bnd:"<<m_bnd; POUTRE_CHECK (m_bnd.contains
    // (shifted), "section(origin,section_bnd) section_bnd shifted by origins is
    // out of bound");
