...
/* template <class T, ptrdiff_t Rank>
 ...
 using cstrided_array_view = strided_array_view<const T, Rank>;*/
    ...
    namespace details
    {
        ...
        using is_viewable_value =
            std::integral_constant<bool, std::is_convertible<std::add_pointer_t<T>, std::add_pointer_t<U>>::value &&
    ...
    template <class T, ptrdiff_t Rank> class array_view
    {
      ...
      public:
        ...
        POUTRE_CXX14_CONSTEXPR array_view(Viewable &&vw) : m_bnd(vw.size()), m_data(vw.data())
        {
        ...
        POUTRE_CONSTEXPR array_view(const array_view<U, R> &rhs) POUTRE_NOEXCEPT : m_bnd(rhs.m_bnd.size()),
                                                                                   m_data(rhs.m_data)
        {
        }
        ...
        POUTRE_CXX14_CONSTEXPR array_view(value_type (&arr)[Extent]) POUTRE_NOEXCEPT : m_bnd(Extent), m_data(nullptr)
        {
            m_bnd[0] = Extent;
            m_data = &arr[0];
        }
        ...
        POUTRE_CXX14_CONSTEXPR array_view<T, Rank - 1> operator[](difference_type slice) const POUTRE_NOEXCEPTONLYNDEBUG
        {
            ...
            idxslice[0] = slice;
            pointer data_slice =
                m_data + details::get_offset_from_coord_nostride<bounds_type, index_type>::op(m_bnd, idxslice);
        ...
        section(const index_type &origin, const bounds_type &section_bnd) const
        {
            ...
            shifted -= 1;
            POUTRE_ASSERTCHECK(m_bnd.contains(shifted), "section(origin,section_bnd) section_bnd shifted by "
                                                        "origins is out of bound");

            pointer data_section =
                m_data + details::get_offset_from_coord_nostride<bounds_type, index_type>::op(m_bnd, origin);
    ...
    template <typename T, std::ptrdiff_t Rank> struct is_strided<strided_array_view<T, Rank>> : std::true_type
    {
    };
