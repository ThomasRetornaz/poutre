...
namespace poutre
  {
	namespace simd
	{
        POUTRE_ALWAYS_INLINE bool IsAligned(const void* ptr, std::size_t alignment=SIMD_IDEAL_MAX_ALIGN_BYTES)//NSIMD_MAX_ALIGNMENT) POUTRE_NOEXCEPTONLYNDEBUG
        {
        ...
        POUTRE_ALWAYS_INLINE bool IsAligned(std::size_t val, std::size_t alignment= SIMD_IDEAL_MAX_ALIGN_BYTES) POUTRE_NOEXCEPTONLYNDEBUG
        {
            POUTRE_ASSERTCHECK(((alignment & (alignment - 1)) == 0),"bad alignment value");
        ...
        template<typename T>
        POUTRE_ALWAYS_INLINE T* t_ReachNextAligned(T* ptr, std::size_t alignment= SIMD_IDEAL_MAX_ALIGN_BYTES) POUTRE_NOEXCEPTONLYNDEBUG
        {
            POUTRE_ASSERTCHECK(((alignment & (alignment - 1)) == 0),"bad alignment value");
      ...
      template<typename T>
      size_t t_ReachNextAlignedSize(size_t size)
      {
            //compute required padding, reach the next multiple aligned
            size_t padd=SIMD_IDEAL_MAX_ALIGN_BYTES/sizeof(T);
            //have a look at http://stackoverflow.com/questions/227897/solve-the-memory-alignment-in-c-interview-question-that-stumped-me for explanation
        ...
        */
        template<typename T>
        const std::pair<ptrdiff_t, ptrdiff_t> POUTRE_ALWAYS_INLINE t_SIMDInputRange(const T* first, const T* last) POUTRE_NOEXCEPTONLYNDEBUG
        {
            POUTRE_ASSERTCHECK(!first,"null ptr");
            POUTRE_ASSERTCHECK(!last,"null ptr");
            const auto simd_size = nsimd::len(nsimd::pack<T>());
        ...
        template<typename T>
        decltype(auto) t_ExpandAligned(scoord ySize,scoord xSize) POUTRE_NOEXCEPT
        {
          return std::vector<T,nsimd::allocator<T>>(ySize*t_ReachNextAlignedSize<T>(xSize));
        }
        /*
