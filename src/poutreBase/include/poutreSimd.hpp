...
namespace poutre {
namespace simd {
POUTRE_ALWAYS_INLINE bool
IsAligned(const void *ptr,
          std::size_t alignment =
              SIMD_IDEAL_MAX_ALIGN_BYTES) // NSIMD_MAX_ALIGNMENT)
                                          // POUTRE_NOEXCEPTONLYNDEBUG
{
  POUTRE_ASSERTCHECK(((alignment & (alignment - 1)) == 0),
                     "bad alignment value");
...
POUTRE_ALWAYS_INLINE bool
IsAligned(std::size_t val, std::size_t alignment = SIMD_IDEAL_MAX_ALIGN_BYTES)
    POUTRE_NOEXCEPTONLYNDEBUG {
  POUTRE_ASSERTCHECK(((alignment & (alignment - 1)) == 0),
                     "bad alignment value");
...
template <typename T>
POUTRE_ALWAYS_INLINE T *
t_ReachNextAligned(T *ptr, std::size_t alignment = SIMD_IDEAL_MAX_ALIGN_BYTES)
    POUTRE_NOEXCEPTONLYNDEBUG {
  POUTRE_ASSERTCHECK(((alignment & (alignment - 1)) == 0),
                     "bad alignment value");
...
template <typename T> size_t t_ReachNextAlignedSize(size_t size) {
  ...
  // for explanation
  return ((static_cast<size_t>(size) + padd - 1) &
          ~(padd - 1)); // from boost\align\detail\align_up.hpp
}
...
*/
template <typename T>
const std::pair<ptrdiff_t, ptrdiff_t> POUTRE_ALWAYS_INLINE
t_SIMDInputRange(const T *first, const T *last) POUTRE_NOEXCEPTONLYNDEBUG {
