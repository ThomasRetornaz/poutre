...
namespace poutre
{
  ...
  // BASE_API std::unique_ptr<IInterface> Create(const std::initializer_list<size_t>& dims, ImgType
  // imgType, PType ptype);

  //! Factory to build contiguous dense image
  IMP_CORE_API std::unique_ptr<IInterface>
               CreateDense(const std::vector<std::size_t> &dims, CompoundType ctype, PType ptype);
