...
namespace poutre
{
    ...
    IMP_CORE_API void AssertSizesCompatible(const IInterface &i_img1, const IInterface &i_img2,
                                            const std::string &i_msg);

    //! @throw runtime_error if input images  are not compatible @see @c AsTypesCompatible
    IMP_CORE_API void AssertAsTypesCompatible(const IInterface &i_img1, const IInterface &i_img2,
                                              const std::string &i_msg);

    // TODO dispatch with sparse type
    // BASE_API std::unique_ptr<IInterface> Create(const std::initializer_list<size_t>& dims, ImgType imgType, PType
    // ptype);

    //! Factory to build contiguous dense image
    IMP_CORE_API std::unique_ptr<IInterface> CreateDense(const std::vector<std::size_t> &dims, CompoundType ctype,
                                                         PType ptype);
