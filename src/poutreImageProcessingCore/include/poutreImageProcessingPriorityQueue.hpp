...
namespace poutre
{
    ...
    class PriorityQueue<key, value, order, true, std::enable_if_t<(TypeTraits<key>::quant <= 16), key>>
    {
      public:
        ...
        PriorityQueue()
            : m_maxPriority(static_cast<int>(TypeTraits<Key_Type>::max())), p_stack(m_maxPriority + 1, 0),
              stack(m_maxPriority + 1), m_current_highestPriority(0)
        {
        }
        // see this later
        PriorityQueue(const PriorityQueue &rhs) = delete;
        ...
        bool empty() const
        {
            return (p_stack[(size_t)getHigherPriority()] == 0);
        }
        ...
        //! returns the top element of the priority queue.The top element is greater or at least as great as any other
        ...
        //! removes the top element of a priority queue.The top element is greater or at least as great as any other
        //! element in the priority queue.
        void pop()
        {
            getHigherPriority();
