...
namespace poutre
{
  ...
  class DenseTensor
  {
    ...
    iterator end() POUTRE_NOEXCEPT { return iterator(m_data + m_numelement, m_data); }

    const_iterator cend() const POUTRE_NOEXCEPT { return const_iterator(m_data + m_numelement, m_data); }

    reverse_iterator rbegin() POUTRE_NOEXCEPT { return (reverse_iterator(m_data + m_numelement - 1, m_data)); }
      ...
      , public DenseTensor<valuetype, NumDims, allocator_type_t>
  {
    ...
    std::size_t GetNumDims() const POUTRE_NOEXCEPT override { return parent_template::GetNumDims(); }
    ...
    std::unique_ptr<IInterface> Clone() const override { return std::make_unique<self_type>(*this); }
    ...
    iterator begin() POUTRE_NOEXCEPT { return iterator(parent_template::m_data, parent_template::m_data); }
    ...
    {
      return (reverse_iterator(parent_template::m_data + parent_template::m_numelement - 1, parent_template::m_data));
    }

    const_reverse_iterator crbegin() const POUTRE_NOEXCEPT
    {
      return (const_reverse_iterator(parent_template::m_data + parent_template::m_numelement - 1, parent_template::m_data));
    }
