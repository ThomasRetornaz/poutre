...
enum class PType {
  ...
  PType_GrayUINT8 =
      1 << 2, //!< 8 bits per pixel, unsigned, grayscale data in [0,255]
  PType_GrayINT32 =
      1 << 3, //!< 32 bits per pixel, signed, grayscale data in ]-2^31,+2^31[
...
template <typename value_type, ptrdiff_t Rank>
using compound_pixel = static_array<value_type, Rank>;
...
template <> struct TypeTraits<pUINT8> {
  ...
  POUTRE_ALWAYS_INLINE POUTRE_STATIC_CONSTEXPR storage_type lowest()
      POUTRE_NOEXCEPT {
    return std::numeric_limits<storage_type>::lowest();
  }
  POUTRE_ALWAYS_INLINE POUTRE_STATIC_CONSTEXPR storage_type min()
      POUTRE_NOEXCEPT {
    return std::numeric_limits<storage_type>::min();
  }
  POUTRE_ALWAYS_INLINE POUTRE_STATIC_CONSTEXPR storage_type max()
      POUTRE_NOEXCEPT {
...
template <class valuetype> struct TypeTraits<compound_pixel<valuetype, 3>> {
  ...
  using safe_signed_type =
      compound_pixel<typename TypeTraits<valuetype>::safe_signed_type, 3>;
  using str_type = compound_pixel<typename TypeTraits<valuetype>::str_type, 3>;
  using accu_type =
      compound_pixel<typename TypeTraits<valuetype>::accu_type, 3>;
  ...
  POUTRE_ALWAYS_INLINE POUTRE_STATIC_CONSTEXPR storage_type lowest()
      POUTRE_NOEXCEPT {
    return compound_pixel<valuetype, 3>(
...
template <class valuetype> struct TypeTraits<compound_pixel<valuetype, 4>> {
  ...
  using safe_signed_type =
      compound_pixel<typename TypeTraits<valuetype>::safe_signed_type, 4>;
  using str_type = compound_pixel<typename TypeTraits<valuetype>::str_type, 4>;
  using accu_type =
      compound_pixel<typename TypeTraits<valuetype>::accu_type, 4>;
  ...
  POUTRE_ALWAYS_INLINE POUTRE_STATIC_CONSTEXPR storage_type min()
      POUTRE_NOEXCEPT {
    return compound_pixel<valuetype, 4>(std::numeric_limits<valuetype>::min(),
                                        std::numeric_limits<valuetype>::min(),
                                        std::numeric_limits<valuetype>::min(),
                                        std::numeric_limits<valuetype>::min());
  }
  ...
  POUTRE_ALWAYS_INLINE POUTRE_STATIC_CONSTEXPR storage_type max()
      POUTRE_NOEXCEPT {
    return compound_pixel<valuetype, 4>(std::numeric_limits<valuetype>::max(),
                                        std::numeric_limits<valuetype>::max(),
                                        std::numeric_limits<valuetype>::max(),
                                        std::numeric_limits<valuetype>::max());
  }
