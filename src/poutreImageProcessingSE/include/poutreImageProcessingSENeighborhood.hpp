...
namespace poutre
{
  ...
  namespace se
  {
    // template <std::ptrdiff_t NumDims = 2> struct NeighborlistSE : public IStructuringElement
    //{
    ...
    //    NeighborlistSE(const storage_type &coordinates) : m_coordinates(coordinates)
    //    {
    //    }
    ...
    //     * The parameter @c dimension controls the dimension on which the extension is queried,
    //     while @c direction
    ...
    template<class neighborhood_t> struct neighborhood_t_interface
    {
      template<class index> auto operator()(const index &idx) const
      {
        // here offest is an array_wiew over nl offsets deplacement
        return static_cast<const neighborhood_t *>(this)->offsets();
      }

      template<class index> auto before(const index &idx) const
      {
        return static_cast<const neighborhood_t *>(this)->before_offsets();
      }

      template<class index> auto after(const index &idx) const
      {
        return static_cast<const neighborhood_t *>(this)->after_offsets();
      }
    };
    //! @} // se_nl_grp
  } // namespace se
