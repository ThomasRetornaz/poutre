...
namespace poutre
{
  template<typename T1, typename T2> struct BinOpInf
  {
    ...
    static /*POUTRE_ALWAYS_INLINE*/ T2 process(const T1 &A0, const T1 &A1) { return (T2)std::min<T1>(A0, A1); }
  };
  ...
  template<typename T1, typename T2> struct BinOpSup
  {
    ...
    static /*POUTRE_ALWAYS_INLINE*/ T2 process(const T1 &A0, const T1 &A1) { return (T2)std::max<T1>(A0, A1); }
  };
