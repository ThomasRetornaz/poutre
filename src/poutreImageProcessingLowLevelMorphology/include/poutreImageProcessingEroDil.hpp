...
namespace poutre
{
    ...
    template <se::NeighborListStaticSE nl, typename T1, typename T2, ptrdiff_t Rank, class BinOp>
    struct t_ErodeDilateOpispatcherDenseImage
    {
        static_assert(Rank == se::NeighborListStaticSETraits<nl>::Rank, "SE and view have not the same Rank");
        void operator()(const DenseImage<T1, Rank> &i_vin, DenseImage<T2, Rank> &o_vout)
        {
            AssertSizesCompatible(i_vin, o_vout, "Incompatible Images sizes");
            AssertAsTypesCompatible(i_vin, o_vout, "Incompatible Images types");
            ...
            if (vInbound == vOutbound && stridevIN == stridevOut) // same bound + same stride -> one idx
            {
                ...
                for (; beg1 != end1; ++beg1)
                {
                    ...
                    for (const auto &idxnl : NlList)
                    {
                        ...
                        if (!vInbound.contains(currentidx))
                            continue;
                        val = BinOp::process(val, details::view_access(i_vin.data(), currentidx, i_vin.stride()));
                    }
                    o_vout[details::view_access(i_vin.data(), *beg1, i_vin.stride())] = static_cast<T2>(val);
                }
                // TODO stride==1 in least significant dimension
            }
            else // default two idx
            {
                ...
                for (; beg1 != end1; ++beg1, ++beg2)
                {
                    ...
                    o_vout[details::view_access(i_vin.data(), *beg2, i_vin.stride())] = static_cast<T2>(val);
                }
            }
        }
    };
    ...
    template <se::NeighborListStaticSE nl, typename T1, typename T2, class BinOp>
    struct t_ErodeDilateOpispatcherDenseImage<nl, T1, T2, 2, BinOp>
    {
        static_assert(2 == se::NeighborListStaticSETraits<nl>::Rank,
                      "SE and view have not the same Rank and rank must be equal to 2");
        void operator()(const DenseImage<T1, 2> &i_vin, DenseImage<T2, 2> &o_vout)
        {
            AssertSizesCompatible(i_vin, o_vout, "Incompatible Images sizes");
            AssertAsTypesCompatible(i_vin, o_vout, "Incompatible Images types");
            ...
            for (ptrdiff_t y = 0; y < suroundingHalfSizeY; ++y)
            {
                ...
                {
                    t_ErodeDilateIterateBorderArrayView2DHelper<nl, T1, T2, BinOp>(i_vinbeg, o_voutbeg, xsize, ysize,
                                                                                   stride, x, y);
                }
            }
    ...
    struct t_ErodeDilateOpispatcher<nl, T1, T2, 2, array_view, array_view, BinOp>
    {
        ...
        void operator()(const array_view<T1, 2> &i_vin, array_view<T2, 2> &o_vout)
        {
            ...
            for (ptrdiff_t y = 0; y < suroundingHalfSizeY; ++y)
            {
                for (ptrdiff_t x = 0; x < xsize; ++x)
                {
                    t_ErodeDilateIterateBorderArrayView2DHelper<nl, T1, T2, BinOp>(i_vinbeg, o_voutbeg, xsize, ysize, x,
                                                                                   y);
                }
            }
            // handling the lower lines
    ...
    template <typename T> struct LineBufferShiftAndArithDilateHelperOp
    {
      public:
        ...
        static T process(const T &A0, const T &A1)
        {
            return std::max<T>(A0, A1);
        }
        ...
        static void ShiftRightLeftAndArith(const_linePtr linein, scoord size, scoord nbStepShiftLeft,
    ...
    template <typename TIn, typename TOut, class HelperOp>
    struct t_ErodeDilateOpispatcherDenseImage<se::NeighborListStaticSE::NeighborListStaticSE2DSquare, TIn, TOut, 2,
                                              HelperOp>
    {
        void operator()(const DenseImage<TIn, 2> &i_vin, DenseImage<TOut, 2> &o_vout)
        {
    ...
    template <typename TIn, typename TOut, class HelperOp>
    struct t_ErodeDilateOpispatcherDenseImage<se::NeighborListStaticSE::NeighborListStaticSE2DCross, TIn, TOut, 2,
                                              HelperOp>
    {
        void operator()(const DenseImage<TIn, 2> &i_vin, DenseImage<TOut, 2> &o_vout)
        {
    ...
    template <typename TIn, typename TOut, class HelperOp>
    struct t_ErodeDilateOpispatcherDenseImage<se::NeighborListStaticSE::NeighborListStaticSE2DSeg0, TIn, TOut, 2,
                                              HelperOp>
    {
        void operator()(const DenseImage<TIn, 2> &i_vin, DenseImage<TOut, 2> &o_vout)
        {
    ...
    // FIXME check TIn, Tout equals cv
    template <typename TIn, typename TOut, class HelperOp>
    struct t_ErodeDilateOpispatcherDenseImage<se::NeighborListStaticSE::NeighborListStaticSE2DSeg90, TIn, TOut, 2,
                                              HelperOp>
    {
        void operator()(const DenseImage<TIn, 2> &i_vin, DenseImage<TOut, 2> &o_vout)
        {
    ...
    // FIXME check TIn, Tout equals cv
    template <typename TIn, typename TOut, class HelperOp>
    struct t_ErodeDilateOpispatcherDenseImage<se::NeighborListStaticSE::NeighborListStaticSE2DSeg45, TIn, TOut, 2,
                                              HelperOp>
    {
        void operator()(const DenseImage<TIn, 2> &i_vin, DenseImage<TOut, 2> &o_vout)
        {
    ...
    // FIXME check TIn, Tout equals cv
    template <typename TIn, typename TOut, class HelperOp>
    struct t_ErodeDilateOpispatcherDenseImage<se::NeighborListStaticSE::NeighborListStaticSE2DSeg135, TIn, TOut, 2,
                                              HelperOp>
    {
        void operator()(const DenseImage<TIn, 2> &i_vin, DenseImage<TOut, 2> &o_vout)
        {
    ...
    void t_Dilate(const DenseImage<TIn, Rank> &i_vin, se::NeighborListStaticSE nl, DenseImage<TOut, Rank> &o_vout)
    {
        ...
        case se::NeighborListStaticSE::NeighborListStaticSE2DSquare: {
            using LineOp = LineBufferShiftAndArithDilateHelperOp<TIn>;
            t_ErodeDilateOpispatcherDenseImage<se::NeighborListStaticSE::NeighborListStaticSE2DSquare, TIn, TOut, Rank,
                                               LineOp>
                dispatcher;
            dispatcher(i_vin, o_vout);
        }
        ...
        case se::NeighborListStaticSE::NeighborListStaticSE2DCross: {
            using LineOp = LineBufferShiftAndArithDilateHelperOp<TIn>;
            t_ErodeDilateOpispatcherDenseImage<se::NeighborListStaticSE::NeighborListStaticSE2DCross, TIn, TOut, Rank,
                                               LineOp>
                dispatcher;
            dispatcher(i_vin, o_vout);
        }
        ...
        case se::NeighborListStaticSE::NeighborListStaticSE2DSeg0: {
            using LineOp = LineBufferShiftAndArithDilateHelperOp<TIn>;
            t_ErodeDilateOpispatcherDenseImage<se::NeighborListStaticSE::NeighborListStaticSE2DSeg0, TIn, TOut, Rank,
                                               LineOp>
                dispatcher;
            dispatcher(i_vin, o_vout);
        }
        ...
        case se::NeighborListStaticSE::NeighborListStaticSE2DSeg90: {
            using LineOp = LineBufferShiftAndArithDilateHelperOp<TIn>;
            t_ErodeDilateOpispatcherDenseImage<se::NeighborListStaticSE::NeighborListStaticSE2DSeg90, TIn, TOut, Rank,
                                               LineOp>
                dispatcher;
            dispatcher(i_vin, o_vout);
        }
        ...
        case se::NeighborListStaticSE::NeighborListStaticSE2DSeg45: {
            using LineOp = LineBufferShiftAndArithDilateHelperOp<TIn>;
            t_ErodeDilateOpispatcherDenseImage<se::NeighborListStaticSE::NeighborListStaticSE2DSeg45, TIn, TOut, Rank,
                                               LineOp>
                dispatcher;
            // t_ErodeDilateOpispatcher<se::NeighborListStaticSE::NeighborListStaticSE2DSeg45,
            // TIn, TOut, Rank, ViewIn, ViewOut, BinOpSup<TIn,TOut>> dispatcher;
        ...
        case se::NeighborListStaticSE::NeighborListStaticSE2DSeg135: {
            using LineOp = LineBufferShiftAndArithDilateHelperOp<TIn>;
            t_ErodeDilateOpispatcherDenseImage<se::NeighborListStaticSE::NeighborListStaticSE2DSeg135, TIn, TOut, Rank,
                                               LineOp>
                dispatcher;
            // t_ErodeDilateOpispatcher<se::NeighborListStaticSE::NeighborListStaticSE2DSeg135,
            // TIn, TOut, Rank, ViewIn, ViewOut, BinOpSup<TIn, TOut>> dispatcher;
