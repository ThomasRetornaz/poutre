...
namespace poutre
{
  ...
  {
    /**
     * @brief  Helper function which copy contant of input OIIO object in DenseImage 2D assuming
     * scalar type
     *
    ...
    template<typename T> void FillImageFromOIIOScalar(const std::string &path, DenseImage<T, 2> &im)
    {
      ...
      if( spec.depth > 1 )
      {
        POUTRE_RUNTIME_ERROR(poutre::format("FillImageFromOIIOScalar: wrong number of dim expected 2 found depth dim"));
      }
    ...
    /**
     * @brief  Helper function which copy contant of input OIIO object in DenseImage 2D assuming RGB
     * like color space
     *
     * @tparam T DenseImage pType
     * @param input preconstructed OIIO object
     ...
     * @warning Unsafe method use @c ImageLoader for safety check
     */
    template<typename T> void FillImageFromOIIOCompound3(OIIO::ImageInput &input, DenseImage<compound<T, 3>, 2> &im)
    {
    ...
    /**
     * @brief Helper function which copy contant of input file in DenseImage 2D assuming like RGB
     * color space
     *
     * @tparam T  T DenseImage pType
     * @param path path of the image to load
    ...
    template<typename T> void FillImageFromOIIOCompound3(const std::string &path, DenseImage<compound<T, 3>, 2> &im)
    {
      ...
      if( spec.depth > 1 )
      {
        POUTRE_RUNTIME_ERROR(poutre::format("FillImageFromOIIOCompound3: wrong number of dim expected 2 found depth dim"));
      }
     ...
     * @warning Unsafe method use @c ImageLoader for safety check
     */
    template<typename T> void FillImageFromOIIOCompound4(OIIO::ImageInput &input, DenseImage<compound<T, 4>, 2> &im)
    {
      const OIIO::ImageSpec &spec = input.spec();
      if( spec.nchannels != 4 ) //-V112
      {
        POUTRE_RUNTIME_ERROR(
            (poutre::format("FillImageFromOIIOCompound4: wrong number of channels expected 4 found %d", spec.nchannels)));
      }
    ...
    /**
     * @brief Helper function which copy contant of input file in DenseImage 2D assuming like RGBA
     * color space
     *
     * @tparam T  T DenseImage pType
     * @param path path of the image to load
    ...
    template<typename T> void FillImageFromOIIOCompound4(const std::string &path, DenseImage<compound<T, 4>, 2> &im)
    {
      ...
      if( spec.depth > 1 )
      {
        POUTRE_RUNTIME_ERROR(poutre::format("FillImageFromOIIOCompound4: wrong number of dim expected 2 found depth dim"));
      }
    ...
    struct StoreWithOIIOOptions
    {
       ...
       *
       */
      StoreWithOIIOOptions() : spec(0, 0, 0), autoconvert(true) { spec.attribute("CompressionQuality", 100); }
    ...
    /**
     * @brief  Helper function which copy contant of DenseImage into OIIO object assuming scalar
     * type
     *
     * @tparam T DenseImage pType
    ...
    template<typename T> void StoreWithOIIOScalar(const DenseImage<T, 2> &im, OIIO::ImageOutput &out)
    {
      ...
      if( out.spec().format != OIIO::BaseTypeFromC<T>::value )
      {
        ...
        {                //-V550
          ...
          for( size_t i = 0; i < im.size(); ++i ) normalized[i] = (pDOUBLE(1.0) / (max - min)) * (im[i] - min);
    ...
    /**
     * @brief  Helper function which copy contant of DenseImage into OIIO object assuming RGB like
     * color space
     *
     * @tparam T DenseImage pType
     * @param im Image to copy into OIIO object
     ...
     * @warning Unsafe method use @c ImageWriter for safety check
     */
    template<typename T> void StoreWithOIIOCompound3(const DenseImage<compound<T, 3>, 2> &im, OIIO::ImageOutput &out)
    {
      const OIIO::ImageSpec &spec = out.spec();
    ...
    /**
     * @brief  Helper function which copy contant of DenseImage on disk assuming RGB like color
     * space
     *
     * @tparam T DenseImage pType
     * @param im Image to serialize
    ...
    /**
     * @brief  Helper function which copy contant of DenseImage into OIIO object assuming RGBA like
     * color space
     *
     * @tparam T DenseImage pType
     * @param im Image to copy into OIIO object
    ...
    /**
     * @brief  Helper function which copy contant of DenseImage on disk assuming RGBA like color
     * space
     *
     * @tparam T DenseImage pType
     * @param im Image to serialize
