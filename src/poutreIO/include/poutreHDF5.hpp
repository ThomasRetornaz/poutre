...
namespace poutre
{
  ...
  namespace details // https://support.hdfgroup.org/HDF5/doc1.8/UG/HDF5_Users_Guide.pdf
  {
    ...
    void StoreWithHDF5_helper(const IInterface &iimage, const std::string &file_name, const std::string &data_set_name)
    {
    ...
    template<typename T, ptrdiff_t rank>
    void
    StoreWithHDF53Planes_helper(const IInterface &iimage, const std::string &file_name, const std::string &data_set_name)
    {
      ...
      {
        POUTRE_RUNTIME_ERROR((poutre::format("StoreWithHDF53Planes_helper: HDF5 fail : {%s}", e.getDetailMsg())));
      }
    }

    template<typename T, ptrdiff_t rank>
    void
    StoreWithHDF54Planes_helper(const IInterface &iimage, const std::string &file_name, const std::string &data_set_name)
    {
      ...
      {
        POUTRE_RUNTIME_ERROR((poutre::format("StoreWithHDF54Planes_helper: HDF5 fail : {%s}", e.getDetailMsg())));
      }
    }
    ...
    template<typename T, ptrdiff_t rank> void LoadFromHDF53Planes_helper(IInterface &iimage, const H5::DataSet &data_set)
    {
      ...
      {
        POUTRE_RUNTIME_ERROR((poutre::format("LoadFromHDF53Planes_helper: HDF5 fail : {%s}", e.getDetailMsg())));
      }
    }
