...
void t_ImageToTypedBufferScalar(const DenseImage<valuetype, NumDims> &im,
  ...
  const auto bytesperpixel =
      sizeof(valuetype); // return size in bytes,byte == octet == 8*bits
  const auto bound = im.bound();

  if (bound.size() * bytesperpixel != size_buffer) {
    POUTRE_RUNTIME_ERROR(
        "t_ImageToTypedBufferScalar:Bad size for the output buffer");
  }
...
template <typename valuetype, std::ptrdiff_t NumDims = 2> // default dim == 2
void t_ImageToBufferCompound3(
    const DenseImage<compound_pixel<valuetype, 3>, NumDims> &im,
    const size_t size_buffer, // Size in bytes!!!
    valuetype *buffer) {
  ...
  if (bound.size() * bytesperpixel != size_buffer) {
    POUTRE_RUNTIME_ERROR(
        "t_ImageToBufferCompound3:Bad size for the output buffer");
  }
  // TODO
}
...
// compound 4
template <typename valuetype, std::ptrdiff_t NumDims = 2> // default dim == 2
void t_ImageToBufferCompound4(
    const DenseImage<compound_pixel<valuetype, 4>, NumDims> &im,
    const size_t size_buffer, // Size in bytes!!!
    valuetype *buffer) {
  ...
  if (bound.size() * bytesperpixel != size_buffer) {
    POUTRE_RUNTIME_ERROR(
        "t_ImageToBufferCompound4:Bad size for the output buffer");
  }
  // TODO
}
...
void t_ImageToBufferForceType(const IInterface &i_im,
  ...
  if (imgType != ImgType::ImgType_Dense) {
    POUTRE_RUNTIME_ERROR(
        "t_ImageToBufferForceType : only dense image type is supported");
  }
  auto compoundType = i_im.GetCType();
  if (compoundType != CompoundType::CompoundType_Scalar) // todo!
  {
    POUTRE_RUNTIME_ERROR(
        "t_ImageToBufferForceType : only scalar type is supported");
  }
  typedef typename boost::remove_cv<
      typename boost::remove_pointer<valuetype>::type>::type inputType;
  ...
  default: {
    POUTRE_RUNTIME_ERROR(
        "t_ImageToBufferForceType::Unsupported number of dims");
  }
  }
}
...
void t_ImageFromBufferWithType(DenseImage<valuetype, NumDims> &im,
  ...
  if (bound.size() * bytesperpixel != size_buffer) {
    POUTRE_RUNTIME_ERROR(
        "t_ImageFromBufferWithType:Bad size for the input buffer");
  }
...
template <typename valuetype>
void t_ImageFromBufferWithType(IInterface &o_im, const size_t i_size_buffer,
                               const valuetype *i_buffer) {
  auto imgType = o_im.GetImgType();
  if (imgType != ImgType::ImgType_Dense) {
    POUTRE_RUNTIME_ERROR(
        "t_ImageFromBufferWithType : only dense image type is supported");
  }
  auto compoundType = o_im.GetCType();
  if (compoundType != CompoundType::CompoundType_Scalar) // todo!
  {
    POUTRE_RUNTIME_ERROR(
        "t_ImageFromBufferWithType : only scalar type is supported");
  }
  typedef typename boost::remove_cv<
      typename boost::remove_pointer<valuetype>::type>::type inputType;
  ...
  default:
    POUTRE_RUNTIME_ERROR(
        "t_ImageFromBufferWithType:Unsupported number of dims");
  }
}
