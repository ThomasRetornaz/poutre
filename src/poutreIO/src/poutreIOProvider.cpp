...
namespace poutre
{
    ...
    bool ImageProvider::IsEmpty() const
    {
        return m_container_loader.empty();
    }

    //! Accessor to the image collection @c begin() iterator.
    ImageProvider::const_iterator ImageProvider::begin() const
    {
        return m_container_loader.begin();
    }

    //! Accessor to the image collection @c end() iterator.
    ImageProvider::const_iterator ImageProvider::end() const
    {
        return m_container_loader.end();
    }
    ...
    {
        if (m_container_loader.empty())
            POUTRE_RUNTIME_ERROR("ImageProvider::Fetch container empty");
