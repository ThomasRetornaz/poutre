...
namespace
{
    ...
    std::unique_ptr<IInterface> loappngHelper(const bf::path &i_path)
    {
        ...
        if (infile == 0)
        {
            POUTRE_RUNTIME_ERROR((boost::format("loappngHelper::unable to open image file %s") % i_path).str());
        }
        ...
        if (!png_sig_cmp(sig, 0, 8))
        {
            ...
            POUTRE_RUNTIME_ERROR(
                (boost::format("loappngHelper::it seems that %s is not a valid  png image file") % i_path).str());
        }
        ...
        if (!png_ptr)
        {
            // cppcheck-suppress doubleFree
            fclose(infile);
            POUTRE_RUNTIME_ERROR("loappngHelper::unable to allocate memory for png struct");
        }
        ...
        if (!info_ptr)
        {
            ...
            POUTRE_RUNTIME_ERROR("loappngHelper::unable to allocate memory for png info struct");
        }
        ...
        try
        {
            ...
            case CompoundType::CompoundType_Scalar: {
                ...
                {
                    png_line_pix = reinterpret_cast<pUINT8 *>(row_pointers[linenb]); // current line png
            ...
            case CompoundType::CompoundType_3Planes: {
                auto img_t = dynamic_cast<DenseTensor<compound_pixel<pUINT8, 3>> *>(&(*imgout));
                if (!img_t)
                {
                    POUTRE_RUNTIME_ERROR("helper_fill_image: downcast fail");
                }
            ...
            case CompoundType::CompoundType_4Planes: {
                auto img_t = dynamic_cast<DenseTensor<compound_pixel<pUINT8, 4>> *>(&(*imgout));
                if (!img_t)
                {
                    POUTRE_RUNTIME_ERROR("helper_fill_image: downcast fail");
                }
            ...
            break;
            default:
                POUTRE_RUNTIME_ERROR((boost::format("loappngHelper: not supported CompoundType %s") %
    ...
    void writepngHelper(const bf::path &i_path, const IInterface &i_img)
    {
        if (i_img.GetImgType() != ImgType::ImgType_Dense)
            POUTRE_RUNTIME_ERROR((boost::format("writepnghelper: only denses images are supported")));

        if (i_img.GetNumDims() != 2)
            POUTRE_RUNTIME_ERROR(
                (boost::format("writepnghelper: can only save 2d images not %dd ") % i_img.GetNumDims()));

        if (i_img.GetPType() != PType::PType_GrayUINT8)
            POUTRE_RUNTIME_ERROR((boost::format("writepnghelper: only GrayUINT8 supported not %s ") %
        ...
        if ((i_img.GetCType() != CompoundType::CompoundType_Scalar) &&
            ...
            POUTRE_RUNTIME_ERROR((boost::format("writepnghelper: only scalar/c3/c4 are supported not %s ") %
        ...
        if (infile == 0)
        {
            POUTRE_RUNTIME_ERROR((boost::format("writepngHelper::unable to open image file %s") % i_path).str());
        }
        ...
        if (!png_ptr)
        {
            // cppcheck-suppress doubleFree
            fclose(infile);
            POUTRE_RUNTIME_ERROR((boost::format("writepngHelper: png_create_write_struct failed")).str());
        }
        ...
        if (!info_ptr)
        {
            ...
            POUTRE_RUNTIME_ERROR((boost::format("writepngHelper: png_create_info_struct failed")).str());
        }
        ...
        if (setjmp(png_jmpbuf(png_ptr)))
        {
            ...
            POUTRE_RUNTIME_ERROR((boost::format("writepngHelper:Error during init_io")).str());
        }
        ...
        if (setjmp(png_jmpbuf(png_ptr)))
        {
            ...
            fclose(infile);
            png_destroy_write_struct(&png_ptr, &info_ptr);
            POUTRE_RUNTIME_ERROR((boost::format("writepngHelper:Error during writing header")).str());
        }

        png_uint_32 width = (png_uint_32)i_img.GetCoords()[0], height = (png_uint_32)i_img.GetCoords()[1];
        int interlace_method = PNG_INTERLACE_NONE, compression_method = PNG_COMPRESSION_TYPE_BASE,
        ...
        default: {
            fclose(infile);
            png_destroy_write_struct(&png_ptr, &info_ptr);
            POUTRE_RUNTIME_ERROR((boost::format("writepngHelper:Unsuported Counpound type %s ") %
                                  boost::lexical_cast<std::string>(i_img.GetCType()))
                                     .str());
        }
        }
        png_set_IHDR(png_ptr, info_ptr, width, height, bit_depth, color_type, interlace_method, compression_method,
                     filter_method);
        ...
        if (setjmp(png_jmpbuf(png_ptr)))
        {
            fclose(infile);
            png_destroy_write_struct(&png_ptr, &info_ptr);
            POUTRE_RUNTIME_ERROR((boost::format("writepngHelper:Error during writing bytes")).str());
        }
        ...
        break;
        case CompoundType::CompoundType_3Planes: {
            auto img_t = dynamic_cast<const DenseTensor<compound_pixel<pUINT8, 3>> *>(&(i_img));
            if (!img_t)
            {
                POUTRE_RUNTIME_ERROR("helper_fill_image: downcast fail");
            }
        ...
        break;
        case CompoundType::CompoundType_4Planes: {
            auto img_t = dynamic_cast<const DenseTensor<compound_pixel<pUINT8, 4>> *>(&(i_img));
            if (!img_t)
            {
                POUTRE_RUNTIME_ERROR("helper_fill_image: downcast fail");
            }
        ...
        break;
        default:
            POUTRE_RUNTIME_ERROR((boost::format("writepngHelper: not supported CompoundType %s") %
                                  boost::lexical_cast<std::string>(i_img.GetCType())));
        }
        ...
        if (setjmp(png_jmpbuf(png_ptr)))
        {
            fclose(infile);
            png_destroy_write_struct(&png_ptr, &info_ptr);
            POUTRE_RUNTIME_ERROR((boost::format("writepngHelper:Error during end of write")).str());
        }
...
namespace poutre
{
    namespace details
    {
        ...
        std::unique_ptr<IInterface> IOLoadPng(const bf::path &i_path)
        {
            if (!bf::exists(i_path))
            {
                POUTRE_RUNTIME_ERROR((boost::format("IOLoadPng::unable to load image from path %s") % i_path).str());
            }

            if (!bf::is_regular_file(i_path))
            {
                POUTRE_RUNTIME_ERROR((boost::format("IOLoadPng::%s is not a regular file") % i_path).str());
            }
            ...
            if (extension != "png")
            {
                POUTRE_RUNTIME_ERROR((boost::format("IOLoadPng::wrong extension %s") % i_path).str());
            }
        ...
        void IOSavePng(const bf::path &i_path, const poutre::IInterface &i_img)
        {
            ...
            if (!bf::exists(dir))
            {
                POUTRE_RUNTIME_ERROR((boost::format("IOSavePng:: path don't exist %s") % dir).str());
            }
            ...
            if (extension != "png")
            {
                POUTRE_RUNTIME_ERROR((boost::format("IOSavePng::wrong extension %s") % extension).str());
            }
