...
void ImageFromStringDenseScalarDispatchPTypeHelper(poutre::IInterface &img,
  ...
  if (!img_t)
    POUTRE_RUNTIME_ERROR(
        "ImageFromStringDenseScalarDispatchPType:Dynamic cast failure");
  for (auto &it : (*img_t)) {
    istrm >> tmp;
    // todo check range validity
    if (istrm.fail()) {
      std::string err = "ImageFromStringDenseScalarDispatchPType: unable to "
...
template <size_t dims, typename ptype>
void ImageFromStringDense3PlanesDispatchPTypeHelper(poutre::IInterface &img,
                                                    std::istringstream &istrm) {
  using ImgType =
      DenseImage<typename TypeTraits<compound_pixel<ptype, 3>>::storage_type,
                 dims>;
  ImgType *img_t = dynamic_cast<ImgType *>(&img);
  typename TypeTraits<ptype>::str_type tmp;
  if (!img_t)
...
template <size_t dims, typename ptype>
void ImageFromStringDense4PlanesDispatchPTypeHelper(poutre::IInterface &img,
                                                    std::istringstream &istrm) {
  using ImgType =
      DenseImage<typename TypeTraits<compound_pixel<ptype, 4>>::storage_type,
                 dims>;
  ImgType *img_t = dynamic_cast<ImgType *>(&img);
  typename TypeTraits<ptype>::str_type tmp;
  if (!img_t)
...
void ImageFromStringDense3PlanesDispatchPType(poutre::IInterface &img,
                                              poutre::PType ptype,
                                              std::istringstream &istrm) {
  switch (ptype) {
  ...
  default: {
    POUTRE_RUNTIME_ERROR(
        ("ImageFromStringDenseScalarDispatchPType::Unsupported ptype:" +
...
template <size_t dims>
void ImageFromStringDense4PlanesDispatchPType(poutre::IInterface &img,
                                              poutre::PType ptype,
                                              std::istringstream &istrm) {
  switch (ptype) {
  case PType::PType_GrayUINT8: {
...
template <size_t dims>
void ImageFromStringDenseScalarDispatchPType(poutre::IInterface &img,
                                             PType ptype,
                                             std::istringstream &istrm) {
  switch (ptype) {
  case PType::PType_GrayUINT8: {
...
template <size_t dims>
void ImageFromStringDispatchDense(poutre::IInterface &img, CompoundType cptype,
                                  PType ptype, std::istringstream &istrm) {
  switch (cptype) {
  ...
  default: {
    POUTRE_RUNTIME_ERROR(
        ("ImageFromStringDispatch::Unsupported compound type:" +
...
std::unique_ptr<IInterface> ImageFromString(const std::string &i_str) {
  ...
  if (imgtype != ImgType::ImgType_Dense) {
    std::string err =
        "ImageFromString: ImgType unknown or not supported yet from " + i_str;
    POUTRE_RUNTIME_ERROR(err);
  }
  ...
  if (istrm.fail()) {
    std::string err =
        "ImageFromString: unable to read CompoundType from " + i_str;
    POUTRE_RUNTIME_ERROR(err);
  }
  istrm >> ptype;
  if (istrm.fail()) {
  ...
  default: { POUTRE_RUNTIME_ERROR("Unsupported number of dims"); }
  }
...
template <size_t dims, typename ptype>
void ImageToStringDenseCompoundType_4PlanesDispatchPTypeHelper(
    const poutre::IInterface &img, std::ostringstream &ostrm) {
  using ImgType =
      DenseImage<typename TypeTraits<compound_pixel<ptype, 4>>::storage_type,
                 dims>;
  const ImgType *img_t = dynamic_cast<const ImgType *>(&img);
  if (!img_t)
    POUTRE_RUNTIME_ERROR("ImageFromStringDenseCompoundType_"
...
template <size_t dims, typename ptype>
void ImageToStringDenseCompoundType_3PlanesDispatchPTypeHelper(
    const poutre::IInterface &img, std::ostringstream &ostrm) {
  using ImgType =
      DenseImage<typename TypeTraits<compound_pixel<ptype, 3>>::storage_type,
                 dims>;
...
template <size_t dims, typename ptype>
void ImageToStringDenseScalarDispatchPTypeHelper(const poutre::IInterface &img,
                                                 std::ostringstream &ostrm) {
  using ImgType = DenseImage<typename TypeTraits<ptype>::storage_type, dims>;
  const ImgType *img_t = dynamic_cast<const ImgType *>(&img);
  if (!img_t)
    POUTRE_RUNTIME_ERROR(
        "ImageToStringDenseScalarDispatchPTypeHelper:Dynamic cast failure");
  ...
  //     1];
  for (auto it = (*img_t).cbegin(); it != (*img_t).cend(); ++it) {
    ostrm << (typename poutre::TypeTraits<ptype>::str_type)(*it) << " ";
  }
  // ostrm.seekp(-1, ostrm.cur);
...
template <size_t dims>
void ImageToStringDenseScalarDispatchPType(const poutre::IInterface &img,
                                           PType ptype,
                                           std::ostringstream &ostrm) {
  switch (ptype) {
  case PType::PType_GrayUINT8: {
    ImageToStringDenseScalarDispatchPTypeHelper<dims, poutre::pUINT8>(img,
                                                                      ostrm);
  } break;

  case PType::PType_GrayINT32: {
    ImageToStringDenseScalarDispatchPTypeHelper<dims, poutre::pINT32>(img,
                                                                      ostrm);
  } break;
  case PType::PType_GrayINT64: {
    ImageToStringDenseScalarDispatchPTypeHelper<dims, poutre::pINT64>(img,
                                                                      ostrm);
  } break;
  case PType::PType_F32: {
    ImageToStringDenseScalarDispatchPTypeHelper<dims, poutre::pFLOAT>(img,
                                                                      ostrm);
  } break;
  case PType::PType_D64: {
    ImageToStringDenseScalarDispatchPTypeHelper<dims, poutre::pDOUBLE>(img,
                                                                       ostrm);
  } break;

  default: {
    POUTRE_RUNTIME_ERROR(
        ("ImageToStringDenseScalarDispatchPType::Unsupported pixel type:" +
         boost::lexical_cast<std::string>(ptype)));
  }
  } // switch (ptype)
}
...
template <size_t dims>
void ImageToStringDenseCompoundType_3PlanesDispatchPType(
    const poutre::IInterface &img, PType ptype, std::ostringstream &ostrm) {
  switch (ptype) {
  case PType::PType_GrayUINT8: {
    ImageToStringDenseCompoundType_3PlanesDispatchPTypeHelper<dims,
                                                              poutre::pUINT8>(
        img, ostrm);
  } break;
...
template <size_t dims>
void ImageToStringDenseCompoundType_4PlanesDispatchPType(
    const poutre::IInterface &img, PType ptype, std::ostringstream &ostrm) {
  switch (ptype) {
  case PType::PType_GrayUINT8: {
    ImageToStringDenseCompoundType_4PlanesDispatchPTypeHelper<dims,
                                                              poutre::pUINT8>(
        img, ostrm);
  } break;
...
void ImageToStringDispatchDense(const poutre::IInterface &img,
                                CompoundType cptype, PType ptype,
                                std::ostringstream &ostrm) {
  switch (cptype) {
  case CompoundType::CompoundType_Scalar: {
  ...
  case CompoundType::CompoundType_3Planes: {
    ImageToStringDenseCompoundType_3PlanesDispatchPType<dims>(img, ptype,
                                                              ostrm);
  } break;

  case CompoundType::CompoundType_4Planes: {
    ImageToStringDenseCompoundType_4PlanesDispatchPType<dims>(img, ptype,
                                                              ostrm);
  } break;
...
std::string ImageToString(const poutre::IInterface &i_image) {
  ...
  if (imgtype != ImgType::ImgType_Dense) {
    std::string err =
        "ImageToString: ImgType unknown or not supported yet from" +
        boost::lexical_cast<std::string>(imgtype);
    POUTRE_RUNTIME_ERROR(err);
  }
