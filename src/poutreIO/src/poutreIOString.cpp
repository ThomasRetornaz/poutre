...
namespace poutre
{
    ...
    void ImageFromStringDenseScalarDispatchPTypeHelper(poutre::IInterface &img, std::istringstream &istrm)
    {
        ...
        if (!img_t)
            POUTRE_RUNTIME_ERROR("ImageFromStringDenseScalarDispatchPType:Dynamic cast failure");
        for (auto &it : (*img_t))
        {
            istrm >> tmp;
    ...
    {
        using ImgType = DenseImage<typename TypeTraits<compound<ptype, 3>>::storage_type, dims>;
        ImgType *img_t = dynamic_cast<ImgType *>(&img);
        typename TypeTraits<ptype>::str_type tmp;
        if (!img_t)
            POUTRE_RUNTIME_ERROR("ImageFromStringDenseScalarDispatchPType:Dynamic cast failure");
    ...
    {
        using ImgType = DenseImage<typename TypeTraits<compound<ptype, 4>>::storage_type, dims>;
        ImgType *img_t = dynamic_cast<ImgType *>(&img);
        typename TypeTraits<ptype>::str_type tmp;
        if (!img_t)
            POUTRE_RUNTIME_ERROR("ImageFromStringDenseScalarDispatchPType:Dynamic cast failure");
    ...
    void ImageFromStringDense3PlanesDispatchPType(poutre::IInterface &img, poutre::PType ptype,
                                                  std::istringstream &istrm)
    {
        ...
        break;
        default: {
            POUTRE_RUNTIME_ERROR(("ImageFromStringDenseScalarDispatchPType::Unsupported ptype:" +
    ...
    template <size_t dims>
    void ImageFromStringDense4PlanesDispatchPType(poutre::IInterface &img, poutre::PType ptype,
                                                  std::istringstream &istrm)
    {
        switch (ptype)
        {
    ...
    template <size_t dims>
    void ImageFromStringDispatchDense(poutre::IInterface &img, CompoundType cptype, PType ptype,
                                      std::istringstream &istrm)
    {
        switch (cptype)
        {
    ...
    template <size_t dims, typename ptype>
    void ImageToStringDenseCompoundType_4PlanesDispatchPTypeHelper(const poutre::IInterface &img,
    ...
    template <size_t dims, typename ptype>
    void ImageToStringDenseCompoundType_3PlanesDispatchPTypeHelper(const poutre::IInterface &img,
                                                                   std::ostringstream &ostrm)
    {
        using ImgType = DenseImage<typename TypeTraits<compound<ptype, 3>>::storage_type, dims>;
        const ImgType *img_t = dynamic_cast<const ImgType *>(&img);
    ...
    void ImageToStringDenseScalarDispatchPTypeHelper(const poutre::IInterface &img, std::ostringstream &ostrm)
    {
        ...
        const ImgType *img_t = dynamic_cast<const ImgType *>(&img);
        if (!img_t)
            POUTRE_RUNTIME_ERROR("ImageToStringDenseScalarDispatchPTypeHelper:Dynamic cast failure");
    ...
    void ImageToStringDenseScalarDispatchPType(const poutre::IInterface &img, PType ptype, std::ostringstream &ostrm)
    {
        ...
        default: {
            POUTRE_RUNTIME_ERROR(("ImageToStringDenseScalarDispatchPType::Unsupported pixel type:" +
                                  boost::lexical_cast<std::string>(ptype)));
        }
        } // switch (ptype)
    }
    ...
    template <size_t dims>
    void ImageToStringDenseCompoundType_3PlanesDispatchPType(const poutre::IInterface &img, PType ptype,
                                                             std::ostringstream &ostrm)
    {
        switch (ptype)
        {
    ...
    template <size_t dims>
    void ImageToStringDenseCompoundType_4PlanesDispatchPType(const poutre::IInterface &img, PType ptype,
                                                             std::ostringstream &ostrm)
    {
        switch (ptype)
        {
    ...
    template <size_t dims>
    void ImageToStringDispatchDense(const poutre::IInterface &img, CompoundType cptype, PType ptype,
                                    std::ostringstream &ostrm)
    {
        switch (cptype)
        {
