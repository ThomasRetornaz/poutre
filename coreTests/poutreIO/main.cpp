...
#define BOOST_TEST_MODULE poutre io
#include "main.hpp"
...
";
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(dims.begin(), dims.end(), expectedcoords.begin(), expectedcoords.end());
    std::vector<poutre::pUINT8> expectedvalues={1,2,3,4,10,12,13,14,20,22,23,24};
    poutre::DenseImage<poutre::pUINT8,2>* img_t=dynamic_cast<poutre::DenseImage<poutre::pUINT8,2>*>(&(*img));
    BOOST_REQUIRE(img_t);
...
";
    ...
    std::vector<size_t> expectedcoords={ 3, 4 };
    BOOST_CHECK_EQUAL_COLLECTIONS(dims.begin(), dims.end(), expectedcoords.begin(), expectedcoords.end());
    std::vector<poutre::pINT32> expectedvalues={-1,2,3,4,10,12,13,14,20,22,23,2400};
    poutre::DenseImage<poutre::pINT32,2>* img_t=dynamic_cast<poutre::DenseImage<poutre::pINT32,2>*>(&(*img));
    BOOST_REQUIRE(img_t);
//    for(const auto& val : (*img_t))
//        {
//        std::cout<<"\nval\t"<<(poutre::TypeTraits<poutre::pUINT8>::str_type)(val);
...
//          {
//              std::cout<<" "<<(poutre::TypeTraits<poutre::pUINT8>::str_type)(val);
//          }
//        }
...
";
    ...
    BOOST_CHECK_EQUAL(img->GetCType(), poutre::CompoundType::CompoundType_4Planes);
    BOOST_CHECK_EQUAL(img->GetNumDims(), 2);
    auto dims=img->GetCoords();
    std::vector<size_t> expectedcoords={ 3, 4 };
    BOOST_CHECK_EQUAL_COLLECTIONS(dims.begin(), dims.end(), expectedcoords.begin(), expectedcoords.end());
